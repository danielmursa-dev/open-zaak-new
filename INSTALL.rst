...
2. Get the code:
   ...
   .. code-block:: bash
       ...
       git clone git@bitbucket.org:maykinmedia/openzaak_new.git
       cd openzaak_new
...
``src/openzaak_new/conf/local.py``. You can base this file on the
...
.. code-block:: bash
    ...
    python src/manage.py test openzaak_new
...
* ``DB_NAME``: name of the database for the project. Defaults to ``openzaak_new``.
* ``DB_USER``: username to connect to the database with. Defaults to ``openzaak_new``.
* ``DB_PASSWORD``: password to use to connect to the database. Defaults to ``openzaak_new``.
...
2. Start the database and web services:
   ...
   .. code-block:: bash
       ...
       docker-compose up -d
       Starting openzaak_new_db_1 ... done
       Starting openzaak_new_web_1 ... done
   ...
   .. code-block:: bash
       ...
       docker logs -f openzaak_new_web_1
...
3. Create an admin user and load initial data. If different container names
   ...
   .. code-block:: bash
       ...
       docker exec -it openzaak_new_web_1 /app/src/manage.py createsuperuser
       Username: admin
       ...
       Superuser created successfully.

       docker exec -it openzaak_new_web_1 /app/src/manage.py loaddata admin_index groups
...
.. _Github: https://github.com/maykinmedia/openzaak_new/
...
environment variables. See ``src/openzaak_new/conf/docker.py`` for
all settings.

.. code-block:: bash

    docker build -t openzaak_new
    docker run \
        -p 8000:8000 \
        -e DATABASE_USERNAME=... \
        -e DATABASE_PASSWORD=... \
    ...
    docker exec -it openzaak_new /app/src/manage.py createsuperuser
